...
const MessageDetail = ({ match, socket, history }) => {
	...
	useEffect(() => {
		...
		getChatroomsById(chatroomId)
			.then((res) => {
				...
				getJobResponseByJob(id[1], res.data.userId[1])
					.then((res) => {
						console.log(res);
				...
				// Get Sp
				getUserInfoById(res.data.userId[1])
					.then((res) => {
						console.log(res);
						setSP(res.data);
					})
		...
		getAllMessages(chatroomId)
			...
			.catch((err) => console.log(err));
		getPaymentChatroomById(chatroomId)
			.then((res) => {
		...
		if (socket) {
			if (message === '' && attachments.length === '') {
				...
				attachments.map((url, index) => {
					axios
						.post(
							'https://udukku.herokuapp.com/chatroom/save-attachment',
							{
								chatroomId,
								attachmentUrl: url,
	...
	const sendAttachment = (e) => {
		...
		axios
			.post(
				'https://udukku.herokuapp.com/api/upload_attachment',
				formData
			)
			.then((res) => {
									...
									<div className='dHIde mt-3'>
										<p style={{ fontSize: '14px' }}>
											<b style={{ color: '#0070f3' }}>
												{chatroom != undefined &&
												chatroom.cost !== undefined
													? 'Funded'
													: 'Quoted Price'}
											</b>
											<i
												className='fas fa-rupee-sign ml-3'
						...
						>
							...
							<div style={{ width: '90%' }}>
								<div className='card-body'>
									...
									>
										<i
											className='fa fa-paperclip dHIde1'
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div style={{ marginLeft: "40px" }}>
					...
					<div className='col-md-4'>
								...
								{chatroom !== undefined ? (
									chatroom.deliverables.length !== 0 ? (
										<>
											...
											<hr className='horizontal-rule' />
											{chatroom.deliverables.map(
												(d, index) =>
													d.secure_url.indexOf(
														'png'
													) !== -1 ||
													d.secure_url.indexOf(
														'jpg'
						...
						{chatroom !== undefined &&
						chatroom.documentation !== undefined &&
